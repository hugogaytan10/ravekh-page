  ...
  useEffect(() => {
    getOrderDetails(orderId || "", context.user.Token || "").then((data) => {
        ...
        const total = data.orderDetails.reduce(
          (acc: number, detail: any) => acc + detail.Price * detail.Item_Quantity,
          0
        );
        setTotal(total);
      }
  ...
  if (!order) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-lg text-gray-700">Cargando detalles de la orden...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <header className="flex items-center bg-gray-800 text-white py-4 px-6">
        <button onClick={() => navigate(-1)} className="mr-4 text-white">Volver</button>
      ...
      <section className="my-6">
        ...
        <div className="bg-gray-100 p-4 rounded">
          ...
          <p>
            <span className="font-semibold">Método de Pago:</span> {order.PaymentMethod || "No especificado"}
          </p>
        </div>
      </section>
      ...
      <footer className="flex justify-between mt-6">
        ...
        <button
          onClick={()=> {PrintTicket(context.store.Name, cart, total, 0)}}
