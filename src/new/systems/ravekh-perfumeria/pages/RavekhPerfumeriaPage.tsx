...
function Pricing() {
  const plans = [
    { name: "Básico", tagline: "Para empezar a ordenar", price: "$XXX", period: "/mes", features: ["Catálogo digital con link único", "Hasta XX productos", "Categorías básicas", "Botón de WhatsApp en cada producto", "Carga inicial de productos"], cta: "Comenzar", highlight: false },
    { name: "Pro", tagline: "Para perfumerías con más volumen", price: "$XXX", period: "/mes", features: ["Todo lo del plan Básico", "Productos ilimitados", "Variantes (tamaño, presentación)", "Productos destacados", "Acompañamiento para compartir mejor", "Actualizaciones prioritarias"], cta: "Comenzar Pro", highlight: true },
    { name: "POS + Catálogo", tagline: "Catálogo + control de ventas", price: "$XXX", period: "/mes", features: ["Todo lo del plan Pro", "Control de ventas e inventario", "Registro de pedidos", "Reportes básicos", "Soporte dedicado"], cta: "Hablar con ventas", highlight: false },
  ];
  return <section id="planes" style={{ background: C.black, padding: "100px 40px" }}><div style={{ maxWidth: 1100, margin: "0 auto" }}><div style={{ textAlign: "center", marginBottom: 72 }}><p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, letterSpacing: 5, color: C.gold, textTransform: "uppercase", marginBottom: 24 }}>Planes</p><h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(30px, 4vw, 52px)", color: C.white, fontWeight: 400, margin: "0 0 16px" }}>Empieza cuando quieras.<br /><em style={{ color: C.gold, fontStyle: "italic" }}>Crece a tu ritmo.</em></h2><p style={{ fontFamily: "Inter, sans-serif", fontSize: 15, color: C.muted, margin: 0 }}>Sin contratos largos. Sin compromisos.</p></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>{plans.map((p) => <div key={p.name} style={{ padding: "44px 32px", border: p.highlight ? `1px solid ${C.gold}` : `1px solid ${C.border}`, background: p.highlight ? "#111008" : C.card, position: "relative", display: "flex", flexDirection: "column" }}>{p.highlight && <div style={{ position: "absolute", top: -1, left: "50%", transform: "translateX(-50%)", background: C.gold, color: C.black, fontFamily: "Inter, sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", padding: "4px 20px" }}>Más popular</div>}<p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, letterSpacing: 3, color: C.gold, textTransform: "uppercase", margin: "0 0 12px" }}>{p.name}</p><h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: C.white, fontWeight: 400, margin: "0 0 8px" }}>{p.tagline}</h3><div style={{ display: "flex", alignItems: "baseline", gap: 4, margin: "24px 0" }}><span style={{ fontFamily: "'Playfair Display', serif", fontSize: 40, color: p.highlight ? C.gold : C.white, fontWeight: 400 }}>{p.price}</span><span style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: C.muted }}>{p.period}</span></div><ul style={{ margin: "0 0 36px", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 12, flex: 1 }}>{p.features.map((f) => <li key={f} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><span style={{ color: C.gold, fontSize: 14, marginTop: 1 }}>✓</span><span style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: C.muted, lineHeight: 1.5 }}>{f}</span></li>)}</ul><a href="#" style={{ display: "block", textAlign: "center", padding: "14px 0", background: p.highlight ? C.gold : "transparent", border: p.highlight ? "none" : `1px solid ${C.border}`, color: p.highlight ? C.black : C.muted, fontFamily: "Inter, sans-serif", fontSize: 12, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase", textDecoration: "none", transition: "all .2s" }}>{p.cta}</a></div>)}</div></div></section>;
}
